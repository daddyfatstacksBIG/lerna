...
describe("conventional-commits", () => {
  describe("recommendVersion()", () => {
    ...
    it("returns next version prerelease bump with prereleaseId", async () => {
      ...
      const bump =
          await recommendVersion(pkg1, "fixed", {prereleaseId : "alpha"});
    ...
    it("returns package-specific prerelease bumps in independent mode with prereleaseId",
       async () => {
         const cwd = await initFixture("independent");
         const [pkg1, pkg2] = await getPackages(cwd);
         const opts = {changelogPreset : "angular"};
         ...
         const [bump1, bump2] = await Promise.all([
           recommendVersion(pkg1, "independent",
                            Object.assign(opts, {prereleaseId : "alpha"})),
           recommendVersion(pkg2, "independent",
                            Object.assign(opts, {prereleaseId : "beta"})),
         ]);
    ...
    it("falls back to patch bumps for non-bumping commit types", async () => {
      ...
      const [pkg1, pkg2] = await getPackages(cwd);
      const opts = {
        // sometimes presets return null for the level, with no actual
        // releaseType...
        changelogPreset : path.resolve(
            __dirname, "__fixtures__/fixed/scripts/null-preset.js"),
      };
    ...
    it("propagates errors from callback", async () => {
      ...
      try {
        await recommendVersion(
            pkg1, "fixed", {changelogPreset : "./scripts/erroring-preset.js"});
      } catch (err) {
    ...
    it("throws an error when an implicit changelog preset cannot be loaded",
       async () => {
         const cwd = await initFixture("fixed");
         const [pkg1] = await getPackages(cwd);
    ...
    it("throws an error when an implicit changelog preset with scope cannot be loaded",
       async () => {
         const cwd = await initFixture("fixed");
         const [pkg1] = await getPackages(cwd);

         try {
           await recommendVersion(pkg1, "fixed",
                                  {changelogPreset : "@scope/garbage"});
         } catch (err) {
           expect(err.message)
               .toMatch(
                   "preset '@scope/garbage' (@scope/conventional-changelog-garbage)");
         }

         expect.hasAssertions();
       });

    it("throws an error when an implicit changelog preset with scoped subpath cannot be loaded",
       async () => {
         const cwd = await initFixture("fixed");
         const [pkg1] = await getPackages(cwd);
    ...
    it("throws an error when an explicit changelog preset cannot be loaded",
       async () => {
         const cwd = await initFixture("fixed");
         const [pkg1] = await getPackages(cwd);
    ...
    it("throws an error when an explicit changelog preset with subpath cannot be loaded",
       async () => {
         const cwd = await initFixture("fixed");
         const [pkg1] = await getPackages(cwd);
    ...
    describe("prerelease bumps", () => {
      ...
      it("prepatch + fix/minor/major => prerelease/preminor/premajor",
         async () => {
           expect(await recommend("fix: changed", {
             initVersion : "1.0.1-beta.0"
           })).toBe("1.0.1-beta.1");
           expect(await recommend("feat: changed")).toBe("1.1.0-beta.0");
           expect(await recommend("feat: changed\n\nBREAKING CHANGE: changed"))
               .toBe("2.0.0-beta.0");
      ...
      it("preminor + fix/minor/major => prerelease/prerelease/premajor",
         async () => {
           expect(await recommend("fix: changed", {
             initVersion : "1.1.0-beta.0"
           })).toBe("1.1.0-beta.1");
  ...
  describe("updateChangelog()", () => {
    ...
    it("creates files if they do not exist", async () => {
      ...
      expect(leafChangelog.logPath)
          .toBe(path.join(pkg1.location, "CHANGELOG.md"));
      expect(rootChangelog.logPath)
          .toBe(path.join(rootPkg.location, "CHANGELOG.md"));
    ...
    it("updates fixed changelogs", async () => {
      ...
      const [leafChangelogContent, rootChangelogContent] = await Promise.all([
        updateChangelog(pkg1, "fixed", /* default preset */ {})
            .then(getFileContent),
    ...
    it("supports custom tagPrefix in fixed mode", async () => {
      ...
      const lastRootChangelog =
          await updateChangelog({location : cwd}, "root", {
            tagPrefix : "dragons-are-awesome",
            version : "1.0.2",
          });
