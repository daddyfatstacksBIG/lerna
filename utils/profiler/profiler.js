...
const hrtimeToMicroseconds =
    hrtime => { return (hrtime[0] * 1e9 + hrtime[1]) / 1000; };

const range = len => { return Array(len).fill().map((_, idx) => idx); };
...
class Profiler {
  constructor(opts) {
    ...
    this.events = [];
    this.logger = log;
    this.outputPath = upath.join(upath.resolve(outputDirectory || "."),
  ...
  output() {
    return fs.outputJson(this.outputPath, this.events)
        .then(
            () => this.logger.info(
                "profiler", `Performance profile saved to ${this.outputPath}`));
  }
}
