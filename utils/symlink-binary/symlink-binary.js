...
function symlinkBinary(srcPackageRef, destPackageRef) {
  return Promise
      .all([ Package.lazy(srcPackageRef), Package.lazy(destPackageRef) ])
      .then(([ srcPackage, destPackage ]) => {
        const actions = Object.keys(srcPackage.bin).map(name => {
          ...
          // Symlink all declared binaries, even if they don't exist (yet). We
          // will assume the package author knows what they're doing and that
          // the binaries will be generated during a later build phase
          // (potentially source compiled from another language).
          return {src, dst};
        });
        ...
        return fs.mkdirp(destPackage.binLocation)
            .then(() => pMap(actions, meta => {
                    if (meta) {
