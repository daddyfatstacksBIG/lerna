...
function symlinkDependencies(packages, packageGraph, tracker) {
  ...
  const nodes = packageGraph.size === packages.length
                    ? packageGraph.values()
        ...
        return pMap(
                   ...
                   ([ dependencyName, resolved ]) => {
                     ...
                     const targetDirectory =
                         path.join(currentNodeModules, dependencyName);
                     ...
                     chain = chain.then(dirExists => {
                       if (dirExists) {
                         ...
                         if (isDepSymlink !== false &&
                             isDepSymlink !== dependencyNode.location) {
                           // installed dependency is a symlink pointing to a
                           // different location
                           tracker.warn(
                               "EREPLACE_OTHER",
                               `Symlink already exists for ${
                                   dependencyName} dependency of ${
                                   currentName}, ` +
                           ...
                           tracker.warn(
                               "EREPLACE_EXIST",
                               `${dependencyName} is already installed for ${
                                   currentName}. Replacing with symlink...`);

                           // remove installed dependency
                           return fs.remove(targetDirectory);
                         }
                       } else {
                         // ensure destination directory exists (dealing with
                         // scoped subdirs)
                     ...
                     const dependencyLocation =
                         dependencyNode.pkg.contents
                             ? path.resolve(dependencyNode.location,
                                            dependencyNode.pkg.contents)
                             : dependencyNode.location;
                     chain = chain.then(() => createSymlink(dependencyLocation,
                                                            targetDirectory,
                                                            "junction"));

                     // TODO: pass PackageGraphNodes directly instead of
                     // Packages
                     chain = chain.then(() => symlinkBinary(dependencyNode.pkg,
                                                            currentNode.pkg));
