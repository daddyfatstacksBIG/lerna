...
function collectUpdates(filteredPackages, packageGraph, execOpts, commandOptions) {
  ...
  if (hasTags(execOpts)) {
    ...
    if (commandOptions.canary) {
      // if it's a merge commit, it will return all the commits that were part
      // of the merge ex: If `ab7533e` had 2 commits, ab7533e^..ab7533e would
      // contain 2 commits + the merge commit
      committish = `${sha}^..${sha}`;
    } else if (!committish) {
      // if no tags found, this will be undefined and we'll use the initial
      // commit
