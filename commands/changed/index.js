...
function factory(argv) { return new ChangedCommand(argv); }
...
class ChangedCommand extends Command {
  ...
  initialize() {
    if (this.options.conventionalGraduate) {
      // provide artificial --conventional-commits so --conventional-graduate
      // works
      this.options.conventionalCommits = true;

      if (this.options.forcePublish) {
        this.logger.warn(
            "option", "--force-publish superseded by --conventional-graduate");
      }
    }
  ...
  execute() {
    ...
    this.logger.success("found", "%d %s ready to publish", this.result.count,
