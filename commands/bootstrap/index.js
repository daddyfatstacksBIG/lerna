...
function factory(argv) { return new BootstrapCommand(argv); }
...
class BootstrapCommand extends Command {
  ...
  initialize() {
    ...
    if (npmClient === "yarn" && this.project.manifest.get("workspaces") &&
    ...
    this.targetGraph = this.options.forceLocal
                           ? new PackageGraph(this.packageGraph.rawPackageList,
    ...
    chain = chain.then(filteredPackages => {
      ...
      if (filteredPackages.length !== this.targetGraph.size &&
        ...
        this.logger.warn(
            "bootstrap",
            "Installing local packages that do not match filters from registry");

        // an explicit --scope, --ignore, or --since should only symlink the
        // targeted packages, no others
        this.targetGraph = new PackageGraph(filteredPackages, "allDependencies",
                                            this.options.forceLocal);

        // never automatically --save or modify lockfiles
        this.npmConfig.npmClientArgs.unshift(
            npmClient === "yarn" ? "--pure-lockfile" : "--no-save");
  ...
  execute() {
    ...
    const packageCountLabel =
        `${filteredLength} package${filteredLength > 1 ? "s" : ""}`;
    ...
    tasks.push(() => this.getDependenciesToInstall(),
               result => this.installExternalDependencies(result),
               () => this.symlinkPackages());

    if (scriptsEnabled) {
      tasks.push(() => this.runLifecycleInPackages("install"),
  ...
  rootHasLocalFileDependencies() {
    const rootDependencies =
        Object.assign({}, this.project.manifest.dependencies);

    return Object.keys(rootDependencies)
        .some(name => this.targetGraph.has(name) &&
                      npa.resolve(name, rootDependencies[name],
  ...
  runLifecycleInPackages(stage) {
    ...
    const mapPackageWithScript = pkg =>
        this.runPackageLifecycle(pkg, stage)
    ...
    const runner =
        this.toposort
            ...
            : pMap(this.filteredPackages, mapPackageWithScript,
                   {concurrency : this.concurrency});
  ...
  hoistedPackageJson(dependency) {
    try {
      ...
      return require(
          path.join(this.hoistedDirectory(dependency), "package.json"));
    } catch (e) {
      // Pass.
      return {};
    }
  ...
  getDependenciesToInstall() {
    ...
    const filteredNodes = new Map(this.filteredPackages.map(
        pkg => [pkg.name, this.targetGraph.get(pkg.name)]));

    // collect root dependency versions
    const mergedRootDeps =
        Object.assign({}, rootPkg.devDependencies, rootPkg.optionalDependencies,
                      rootPkg.dependencies);
    const rootExternalVersions = new Map(
        Object.keys(mergedRootDeps)
            .map(externalName => [externalName, mergedRootDeps[externalName]]));
    ...
    for (const [leafName, leafNode] of filteredNodes) {
      for (const [externalName, resolved] of leafNode.externalDependencies) {
        ...
        const record =
            depsToInstall.get(externalName) ||
            depsToInstall.set(externalName, new Map()).get(externalName);
        const externalDependents =
            record.get(version) || record.set(version, new Set()).get(version);
    ...
    const rootActions = [];
    const leafActions = [];
    // We don't want to exit on the first hoist issue, but log them all and then
    // exit
    ...
    for (const [externalName, externalDependents] of depsToInstall) {
      ...
      if (this.hoisting && isHoistedPackage(externalName, this.hoisting)) {
        const commonVersion =
            Array.from(externalDependents.keys())
                .reduce((a, b) => externalDependents.get(a).size >
                                          externalDependents.get(b).size
                                      ? a
                                      : b);
        ...
        if (rootVersion !== commonVersion) {
          this.logger.warn("EHOIST_ROOT_VERSION",
                           `The repository root depends on ${externalName}@${
                               rootVersion}, ` +
                               `which differs from the more common ${
                                   externalName}@${commonVersion}.`);
        ...
        const dependents =
            Array.from(externalDependents.get(rootVersion))
                .map(leafName => this.targetGraph.get(leafName).pkg);
        ...
        rootActions.push(
            () => hasDependencyInstalled(rootPkg, externalName, rootVersion)
                      .then(isSatisfied => {
                        rootSet.add({
                          name : externalName,
                          dependents,
      ...
      for (const [leafVersion, leafDependents] of externalDependents) {
        for (const leafName of leafDependents) {
          if (rootVersion) {
            this.logger.warn("EHOIST_PKG_VERSION",
                             `"${leafName}" package depends on ${
                                 externalName}@${leafVersion}, ` +
                                 `which differs from the hoisted ${
                                     externalName}@${rootVersion}.`);
            if (this.options.strict) {
              strictExitOnWarning = true;
            }
          }
          ...
          const leafRecord = leaves.get(leafNode) ||
                             leaves.set(leafNode, new Set()).get(leafNode);

          // only install dependency if it's not already installed
          leafActions.push(
              () => hasDependencyInstalled(leafNode.pkg, externalName,
                                           leafVersion)
                        .then(isSatisfied => {
                          leafRecord.add({
    ...
    return pMapSeries([...rootActions, ...leafActions ], el => el())
        .then(() => {
          this.logger.silly("root dependencies",
                            JSON.stringify(rootSet, null, 2));
          this.logger.silly("leaf dependencies",
  ...
  installExternalDependencies({leaves, rootSet}) {
    ...
    if (rootSet.size) {
      ...
      actions.push(() => {
        ...
        const promise = npmInstall.dependencies(rootPkg, depsToInstallInRoot,
                                                this.npmConfig);

        return pulseTillDone(promise)
            .then(() =>
                      // Link binaries into dependent packages so npm scripts
      ...
      actions.push(() => {
        ...
        const candidates =
            root.filter(dep => dep.dependents.length)
                .reduce((list, {name, dependents}) => {
                  const dirs =
                      dependents
                          .filter(pkg => pkg.nodeModulesLocation !==
                                         rootPkg.nodeModulesLocation)
    ...
    leaves.forEach((leafRecord, leafNode) => {
      ...
      if (deps.some(({isSatisfied}) => !isSatisfied)) {
        actions.push(() => {
          ...
          const promise = npmInstall.dependencies(leafNode.pkg, dependencies,
                                                  leafNpmConfig);
    ...
    if (actions.length) {
      ...
      tracker.verbose("actions", "%d actions, concurrency %d", actions.length,
                      this.concurrency);
   ...
   */
  symlinkPackages() {
    return symlinkDependencies(this.filteredPackages, this.targetGraph,
