...
exports.builder = (yargs, composed) => {
  const opts = {
    ...
    "conventional-graduate" : {
      ...
      // type must remain ambiguous because it is overloaded (boolean _or_
      // string _or_ array)
    },
    ...
    // preid is copied into ../publish/command because a whitelist for one
    // option isn't worth it
    preid : {
      describe :
          "Specify the prerelease identifier when versioning a prerelease",
      type : "string",
      requiresArg : true,
    ...
    "tag-version-prefix" : {
      describe :
          "Customize the tag prefix. To remove entirely, pass an empty string.",
      type : "string",
      requiresArg : true,
  ...
  if (composed) {
    ...
    Object.keys(opts).forEach(key => { opts[key].hidden = true; });
  ...
  return yargs
      ...
      .check(argv => {
        ...
        if (argv.cdVersion && !argv.bump) {
          ...
          log.warn("deprecated",
                   "--cd-version has been replaced by positional [bump]");
        }
        ...
        if (argv.repoVersion && !argv.bump) {
          ...
          log.warn("deprecated",
                   "--repo-version has been replaced by positional [bump]");
        }
        ...
        if (argv.githubRelease) {
          ...
          log.warn(
              "deprecated",
              "--github-release has been replaced by --create-release=github");
        }
        ...
        if (argv["--"]) {
          log.warn(
              "EDOUBLEDASH",
              "Arguments after -- are no longer passed to subprocess executions.");
          log.warn("EDOUBLEDASH",
                   "This will cause an error in a future major version.");
        }
...
exports.addBumpPositional = function addBumpPositional(
    yargs, additionalKeywords = []) {
  ...
  yargs.positional("bump", {
    ...
    coerce : choice => {
      if (!semver.valid(choice) && semverKeywords.indexOf(choice) === -1) {
        throw new Error(`bump must be an explicit version string _or_ one of: ${
            bumpOptionList}`);
      }
