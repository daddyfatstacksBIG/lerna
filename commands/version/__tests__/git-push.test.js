...
async function listRemoteTags(cwd) {
  return execa.stdout("git", [ "ls-remote", "--tags", "--refs", "--quiet" ],
                      {cwd});
}

beforeEach(() => { jest.spyOn(childProcess, "exec"); });
...
test("gitPush", async () => {
  ...
  expect(childProcess.exec)
      .toHaveBeenLastCalledWith("git",
                                [
                                  "push", "--follow-tags", "--no-verify",
                                  "--atomic", "origin", "master"
                                ],
                                {cwd});
...
test("unexpected git error", async () => {
  ...
  await expect(gitPush("origin", "master", {cwd}))
