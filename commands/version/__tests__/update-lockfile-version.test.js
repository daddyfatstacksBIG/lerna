...
test("updateLockfileVersion", async () => {
  ...
  expect(returnedLockfilePath)
      .toBe(path.join(pkg.location, "package-lock.json"));
  expect(Array.from(loadJsonFile.registry.keys())).toStrictEqual([
    "/packages/package-1"
  ]);
  expect(fs.readJSONSync(returnedLockfilePath))
      .toHaveProperty("version", "2.0.0");
});
...
test("updateLockfileVersion without sibling lockfile", async () => {
  ...
  expect(fs.pathExistsSync(path.join(pkg.location, "package-lock.json")))
      .toBe(false);
});
